 ...
 * or photo. They call gv_lead() instead. To build a demo for a real firm, the
 ...
 *
 * With no lead-data.php present the defaults below render the fictional G.V Rörservice AB.
 *
 ...
 * @return string
 */
function gv_sample_note() {
	return gv_lead( 'ar_riktig' ) ? '' : ' <span class="gv-sample">(platshållare)</span>';
}
 ...
 * @return string File name for gv_lead_img().
 */
function gv_lead_bild( $i ) {
	$bilder = (array) gv_lead( 'bilder', array() );
 ...
 * @return string
 */
function gv_ring_label() {
	$namn = gv_lead( 'kontakt' );
	return $namn ? sprintf( 'Ring %s · %s', $namn, gv_lead( 'telefon' ) ) : 'Ring ' . gv_lead( 'telefon' );
}
