<?php
/**
 * G.V Rörservice AB theme functions.
 *
 * Kept short on purpose. Design lives in theme.json, markup in templates and patterns.
 * Lead capture lives in the "G.V Rörservice AB Leads" plugin, so leads survive a theme change.
 *
 ...
 * Patterns read these through gv_lead() so one demo can be rebuilt for any firm.
 */
...
add_action(
	'wp_enqueue_scripts',
	function () {
		wp_enqueue_style( 'gvrorservice', get_stylesheet_uri(), array(), wp_get_theme()->get( 'Version' ) );
	}
);
add_action(
	'after_setup_theme',
...
add_action(
	'init',
	function () {
		register_block_style( 'core/paragraph', array( 'name' => 'eyebrow', 'label' => __( 'Eyebrow label', 'gvrorservice' ) ) );
		register_block_style( 'core/list', array( 'name' => 'checklist', 'label' => __( 'Tick list', 'gvrorservice' ) ) );

		register_block_pattern_category( 'gvrorservice-sections', array( 'label' => __( 'G.V Rörservice AB sections', 'gvrorservice' ) ) );
		register_block_pattern_category( 'gvrorservice-pages', array( 'label' => __( 'G.V Rörservice AB page layouts', 'gvrorservice' ) ) );
	}
);
