...
add_action(
	...
	function () {
		register_post_type(
			...
			array(
				'labels'          => array(
					'name'          => __( 'Leads', 'gvrorservice-leads' ),
					'singular_name' => __( 'Lead', 'gvrorservice-leads' ),
					'edit_item'     => __( 'Förfrågan', 'gvrorservice-leads' ),
					'search_items'  => __( 'Search leads', 'gvrorservice-leads' ),
					'not_found'     => __( 'Inga förfrågningar än. Skicka formuläret så dyker den upp här.', 'gvrorservice-leads' ),
					'all_items'     => __( 'Alla förfrågningar', 'gvrorservice-leads' ),
				),
...
add_filter(
	...
	function () {
		return array(
			...
			'title'        => __( 'Name', 'gvrorservice-leads' ),
			'npl_phone'    => __( 'Phone', 'gvrorservice-leads' ),
			'npl_service'  => __( 'Needs', 'gvrorservice-leads' ),
			'npl_timeline' => __( 'When', 'gvrorservice-leads' ),
			'npl_zip'      => __( 'ZIP', 'gvrorservice-leads' ),
			'npl_source'   => __( 'Source', 'gvrorservice-leads' ),
			'npl_status'   => __( 'Status', 'gvrorservice-leads' ),
			'date'         => __( 'Received', 'gvrorservice-leads' ),
		);
	}
);
...
function npl_source_summary( $post_id ) {
	...
	return __( 'Direct', 'gvrorservice-leads' );
}
...
add_action(
	...
	function () {
		add_meta_box( 'npl_details', __( 'What they told us', 'gvrorservice-leads' ), 'npl_details_box', 'npl_lead', 'normal', 'high' );
		add_meta_box( 'npl_status', __( 'Status', 'gvrorservice-leads' ), 'npl_status_box', 'npl_lead', 'side', 'high' );
	}
);
...
function npl_details_box( $post ) {
	$rows = array(
		__( 'Name', 'gvrorservice-leads' )     => get_post_meta( $post->ID, 'name', true ),
		__( 'Phone', 'gvrorservice-leads' )    => get_post_meta( $post->ID, 'phone', true ),
		__( 'E-post', 'gvrorservice-leads' )    => get_post_meta( $post->ID, 'email', true ),
		__( 'ZIP', 'gvrorservice-leads' )      => get_post_meta( $post->ID, 'zip', true ),
		__( 'Needs', 'gvrorservice-leads' )    => npl_label( 'service', get_post_meta( $post->ID, 'service', true ) ),
		__( 'When', 'gvrorservice-leads' )     => npl_label( 'timeline', get_post_meta( $post->ID, 'timeline', true ) ),
		__( 'Source', 'gvrorservice-leads' )   => npl_source_summary( $post->ID ),
	);
	...
	$rows[ __( 'Samtycke lämnat', 'gvrorservice-leads' ) ] = get_post_meta( $post->ID, 'consent_at', true ) . ' — "' . get_post_meta( $post->ID, 'consent_text', true ) . '"';
...
function npl_status_box( $post ) {
	...
	echo '<label class="screen-reader-text" for="npl-status">' . esc_html__( 'Status', 'gvrorservice-leads' ) . '</label><select id="npl-status" name="npl_status" style="width:100%">';
