...
add_filter(
	...
	function ( $exporters ) {
		$exporters['gvrorservice-leads'] = array(
			'exporter_friendly_name' => __( 'Quote requests', 'gvrorservice-leads' ),
			'callback'               => function ( $email ) {
				$items = array();
				foreach ( npl_leads_by_email( $email ) as $lead ) {
					$data = array( array( 'name' => __( 'Received', 'gvrorservice-leads' ), 'value' => $lead->post_date ) );
					...
					$items[] = array(
						'group_id'    => 'gvrorservice-leads',
						'group_label' => __( 'Quote requests', 'gvrorservice-leads' ),
...
add_filter(
	...
	function ( $erasers ) {
		$erasers['gvrorservice-leads'] = array(
			'eraser_friendly_name' => __( 'Quote requests', 'gvrorservice-leads' ),
			'callback'             => function ( $email ) {
				$removed = false;
				foreach ( npl_leads_by_email( $email ) as $lead ) {
